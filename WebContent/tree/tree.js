...
function Tree(tid, rootName)
{
    ...
    * Font name
    */
   this.fontName = "Verdana"; //-della modify
   /**
    * Directory for tree icon, must absolute path from webroot
    */
   this.iconDir = "tree/images/treeicon/win2000";//-- della modify
   /**
   ...
   this.leftSideForIE = function(node)
   {
	  ...
	  renderString = renderString + "<SPAN style='color:" + fg + ";background-color:" + bg + ";font-size:" + this.fontSize + ";font-family:" + this.fontName + ";cursor:hand'" + " onClick='nodes[" + node.index + "].tree.treeSelectionListener(nodes[" + node.index + "])'><a href='./main.jsp?MacAddr="+node.name+"' target='main'>" + node.name + "</a></SPAN>" //-della modify
      return renderString;
   }
   /**
    * generate leftside
